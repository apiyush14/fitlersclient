...
const updateRetryTimer=()=>{
 setRetryOtpTimer((prevState)=>{
  console.log("==============Update Retry Timer====================");
  return prevState-1;
 });
};
...
const onClickSubmitOTP=()=>{
 dispatch(authActions.validateOTPForMSISDN(MSISDN,otpCode)).then((response)=>{
           ...
           console.log('===================Inside Submit OTP Pressed=================');
           console.log(retryTimerId);
...
return (
  ...
  style={styles.bgImage}>
   ...
   {!isValidMSISDN?(
   <Text style={styles.errorTextMSISDN}>Please enter valid phone number</Text>):(<Text/>)}
   <RoundButton 
